...
export function UploadWidget(props: UploadWidgetProps) {
  ...
  const showSizeError = useCallback(
    (file?: File | null) => {
      ...
      const sizeText = file && typeof file.size === 'number' ? `Selected: ${formatFileSize(file.size)}.` : ''
      setResult({
        kind: 'error',
  ...
  const onSubmit = useCallback<React.FormEventHandler<HTMLFormElement>>(
    async (e) => {
      ...
      try {
        ...
        setResult({ kind: 'success', text: `✅ Success! ${successFileName} uploaded${successCategory}.` })
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err)
        setResult({ kind: 'error', text: `❌ ERROR!\n\n${msg}\n\nCheck the console for more details.` })
    ...
    [category, claimId, claimName, fullName, maxFileSizeBytes, selectedFile, showSizeError, token, uploadUrl, userId]
  )
  ...
  return (
    <div>
        ...
        <form className="upload-form" onSubmit={onSubmit}>
          <div className="form-group">
            <label htmlFor="categorySelect">
              Category <span className="required" aria-hidden="true">* Required</span>
            </label>
            <select
              id="categorySelect"
          ...
          >
            <div className="drop-content">
              <div className="upload-icon" aria-hidden="true">
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M12 5v10M12 5l-4 4M12 5l4 4M5 19h14" stroke="#0f766e" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </div>
            ...
            <input ref={inputRef} type="file" className="file-input" required aria-required="true" onChange={onInputChange} />
          </div>
      ...
      <div
        className={`result${result.kind === 'success' ? ' success' : result.kind === 'error' ? ' error' : ''}`}
